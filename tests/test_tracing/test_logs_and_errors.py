...
def test_record_error_marks_the_span_and_captures_exception_info(
    ...
    with pytest.raises(TimeoutError), span_scope("stripe_api_call") as span:
...
def test_error_propagates_to_every_enclosing_span(trace: Trace) -> None:
    ...
    with pytest.raises(TimeoutError), span_scope("handle_order") as root:
...
def test_sibling_of_a_failed_span_is_unaffected(trace: Trace) -> None:
    with pytest.raises(TimeoutError), span_scope("handle_order"):
...
def test_exception_is_re_raised_unchanged(trace: Trace) -> None:
    ...
    with pytest.raises(TimeoutError) as exc_info, span_scope("stripe_api_call"):
        raise original
...
def test_finished_trace_status_is_ok_with_no_errors(trace: Trace) -> None:
    with span_scope("handle_order"), span_scope("validate_payment"):
        pass
